...
const releases = await fetch("https://api.github.com/repos/Newipe/Dohcord/releases", {
    headers: {
...
const metaInfo = await (async () => {
    for (const release of releases) {
        const metaAsset = release.assets.find((a: any) => a.name === "dev.newipe.Dohcord.metainfo.xml");
...
await fs.writeFile("./dist/dev.newipe.Dohcord.metainfo.xml", output, "utf-8");

console.log("Updated meta information written to ./dist/dev.newipe.Dohcord.metainfo.xml");
